...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '8074095069:AAGWDR3p6ZNtZJCUnDG0o2whcGMwIRToirg'
ADMIN_USER_ID = 7409754329 
ADMIN_USER_ID = 7409754329 
...
PROTECTED_FILES = ["sharp.py", "sharp"]
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot @shivamxraj!*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    argument_type = argument_type["value"] if argument_type else 1  # Default to 3 if not set
